...
export function useSession() {
  const currentSession = useSessionStore(
    (state: CombinedSessionStoreState) => state.currentSession,
  );
  ...
  const recordActivity = useSessionStore(
    (state: CombinedSessionStoreState) => state.recordActivity,
  );
  const clearError = useSessionStore((state: CombinedSessionStoreState) => state.clearError);
  const runDiagnostics = useSessionStore(
    (state: CombinedSessionStoreState) => state.runDiagnostics,
  );
