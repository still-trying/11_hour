 ...
 */
export const stateLogger = <T extends object>(storeName: StoreName, enabled: boolean = true) => {
 ...
 */
export const stateValidator = <T extends object>(storeName: StoreName, schema?: z.ZodSchema<T>) => {
  return (config: StateCreator<T>): StateCreator<T> =>
    (set, get, api) => {
      ...
      const validatedSet = (partial: any, replace?: boolean) => {
        ...
        const nextPartial =
          typeof partial === 'function' ? (partial as any)(currentState) : partial;

        const candidateState = replace ? nextPartial : { ...currentState, ...nextPartial };
 ...
 */
export const stateErrorBoundary = <T extends object>(storeName: StoreName) => {
  return (config: StateCreator<T>): StateCreator<T> =>
    (set, get, api) => {
