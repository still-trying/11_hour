...
class BootstrapEngineClass {
   ...
   * Leverages caching to guarantee the engine runs strictly once.
   */
  async run(): Promise<RuntimeConfig> {
    if (this.initializationPromise) {
      console.info(`${RUNTIME_LOG_PREFIX} Bootstrap already initiated or completed. Reusing active thread.`);
  ...
  private async executeBootstrap(): Promise<RuntimeConfig> {
    ...
    for (const phase of STARTUP_PHASES_ORDER) {
      ...
      try {
        ...
        console.info(`${RUNTIME_LOG_PREFIX} Completed Phase: ${phase} in ${phaseDuration.toFixed(1)}ms`);
      } catch (err) {
        const phaseDuration = performance.now() - phaseStart;
    ...
    console.info(`${RUNTIME_LOG_PREFIX} Application successfully bootstrapped in ${totalDuration.toFixed(1)}ms!`);
    
    // Broadcast app started state safely
    try {
    ...
    switch (phase) {
      case StartupPhase.ENVIRONMENT:
        if (!validateEnvironment()) {
          throw new Error('System-level browser context validation failed (LocalStorage disabled or unavailable).');
        }
        break;
      ...
      case StartupPhase.FIREBASE_PLATFORM:
        // Attempt cloud Firestore connectivity validation
        try {
          await validateFirestoreConnection();
        } catch (err) {
          console.warn(`${RUNTIME_LOG_PREFIX} Firebase remote connection verification skipped or failed, continuing with offline capabilities.`, err);
        }
        break;

      case StartupPhase.THEME_ENGINE:
        // Standard non-blocking verify: ensure window and CSS sync function exists
      ...
      case StartupPhase.STATE_PLATFORM:
        // Graceful hydration check: wait a max of 200ms for stores to hydrate from caches, then proceed anyway
        await new Promise<void>((resolve) => {
          const timeout = setTimeout(() => {
            console.warn(`${RUNTIME_LOG_PREFIX} Store hydration exceeded 200ms fallback time. Continuing startup process offline.`);
            unsubscribe();
            resolve();
          }, 200);
          ...
          const unsubscribe = StoreRegistry.subscribeToHydration((allHydrated) => {
            if (allHydrated) {
              ...
              console.info(`${RUNTIME_LOG_PREFIX} All registered state stores successfully hydrated from caches.`);
              resolve();
            }
          });
        });
      ...
      case StartupPhase.ROUTER:
        // Verify browser navigation capability exists
      ...
      case StartupPhase.RUNTIME_CONTEXT:
        // Context variables compiled successfully
        break;

      case StartupPhase.APPLICATION_MOUNT:
        // Prepare DOM node validation
  ...
  private recordMetric(phase: StartupPhase, durationMs: number, status: 'success' | 'failed', error?: string): void {
    ...
    // Clean duplicate metrics for retries
