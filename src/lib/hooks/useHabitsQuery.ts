...
import { habitsApi, type CreateHabitInput, type UpdateHabitInput } from '@/lib/services/habits';
...
export function useHabitsQuery() {
  ...
  useEffect(() => {
    ...
    // Store the interval ref so we can clean it up on unmount
  ...
  const logsQuery = useQuery({
    ...
    queryFn: () =>
      habitsApi.fetchLogs(format(weekStart, 'yyyy-MM-dd'), format(weekEnd, 'yyyy-MM-dd')),
    enabled: !!user,
  });
