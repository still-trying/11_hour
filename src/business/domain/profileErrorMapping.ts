 ...
 * Maps raw infrastructural or Supabase exceptions to our domain-specific ProfileException.
 */
...
export class ProfileErrorMapper {
  ...
  public static map(
    ...
    error: any,
    defaultCode: ProfileErrorCode = ProfileErrorCode.UNKNOWN,
  ): ProfileException {
    ...
    // Detect Supabase Auth or database-specific errors
    if (error && typeof error === 'object') {
      ...
      if (errCode.includes('permission-denied') || errMsg.includes('insufficient permissions')) {
        ...
        message =
          'Database permission denied. Ensure you are signed in and editing your own profile.';
    ...
    ProfileLogging.error(
      `[CorrelationId: ${correlationId}] Mapped exception code: ${code}. Message: ${message}`,
      error,
    );
