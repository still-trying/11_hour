...
export class SessionPersistence {
   ...
   * Survives standard cache clears by caching securely in localStorage.
   */
  public static getOrCreateDeviceId(): string {
    try {
      ...
      console.warn(
        '⚠️ [SessionPersistence] Storage disabled or sandboxed. Using ephemeral device fallback.',
        e,
      );
  ...
  public static loadFromLocalCache(): ISession | null {
    try {
      ...
      console.error(
        '❌ [SessionPersistence] Cache corruption detected. Resetting session cache.',
        error,
      );
