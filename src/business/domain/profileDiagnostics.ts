...
export class ProfileDiagnostics {
  ...
  public static verifyAndHeal(profile: unknown): IDomainUserProfile {
    try {
      ...
      ProfileLogging.warn(
        'Profile integrity verification failed. Initiating self-healing routine...',
        error,
      );
        ...
        const healed: Record<string, any> = {
          ...
          preferences: {
            ...
            notificationPreferences: {
              ...
              urgencyThreshold:
                raw.preferences?.notificationPreferences?.urgencyThreshold || 'high',
            },
          },
          application: {
          ...
          futureReady: {
            aiPreferences: {
              ...
              temperature:
                typeof raw.futureReady?.aiPreferences?.temperature === 'number'
                  ? raw.futureReady?.aiPreferences?.temperature
                  : 0.7,
              autoRefactorEnabled: !!raw.futureReady?.aiPreferences?.autoRefactorEnabled,
            },
            productivityPreferences: {
              defaultBlockDurationMinutes:
                raw.futureReady?.productivityPreferences?.defaultBlockDurationMinutes || 25,
              dailyFocusGoalMinutes:
                raw.futureReady?.productivityPreferences?.dailyFocusGoalMinutes || 120,
              enableSoundAlerts:
                raw.futureReady?.productivityPreferences?.enableSoundAlerts !== false,
              enableBreakTimer:
                raw.futureReady?.productivityPreferences?.enableBreakTimer !== false,
            },
            personalization: {
              occupation: raw.futureReady?.personalization?.occupation,
              focusAreas: Array.isArray(raw.futureReady?.personalization?.focusAreas)
                ? raw.futureReady?.personalization?.focusAreas
                : [],
